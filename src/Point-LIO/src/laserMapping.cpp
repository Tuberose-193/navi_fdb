...
PointCloudXYZI::Ptr z_cloud_filtered (new PointCloudXYZI());
...
PointCloudXYZI::Ptr loadPointcloudFromPcd(const std::string &filename){
    ...
    if(pcl::io::loadPCDFile<PointType>(filename, *cloud) == -1) {
        RCLCPP_ERROR(rclcpp::get_logger("laserMapping"), "Failed to load PCD file: %s", filename.c_str());
        return cloud;
    }
    RCLCPP_INFO(rclcpp::get_logger("laserMapping"), "Loaded PCD with %d points", cloud->size());
    return cloud;
}
...
void publish_frame_body(rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudFull_body)
{
    ...
    //滤除头顶正上方的点
    pcl::PassThrough<pcl::PointXYZINormal> linear_filter;
    linear_filter.setInputCloud(cloud_cylinderRemoved);
    linear_filter.setFilterFieldName ("z");         // 设置过滤时所依赖的坐标轴字段
    linear_filter.setFilterLimits (0.2, 10.0);
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud_ground_segment(new pcl::PointCloud<pcl::PointXYZINormal>());
    linear_filter.filter (*cloud_ground_segment);

    // 将滤波后的结果点云重新赋值给cloud_filtered
    *cloud_filtered = *cloud_ground_segment;
...
void set_posestamp(T & out)
{
    ...
    {   
        if(is_first_kf_)
        {
            kf_output.x_.pos(0) = 0.0;//lqw switch a init pose
...
private:
    void readParameters()
    {
        ...
        this->declare_parameter<bool>("pcd_save.pcd_save_en", true); //false
    ...
    void timer_callback()
    {
        ...
        if(sync_packages(Measures)) //检查是否成功同步了测量数据（包括激光雷达和IMU数据）
        {
            ...
            */
            ...
            if (init_map) 
            {
                RCLCPP_DEBUG(this->get_logger(), "进入测距逻辑");
                Eigen::Vector3d current_position = (!use_imu_as_input) ? kf_output.x_.pos : kf_input.x_.pos;
                //计算水平距离
                ...
                //计算垂直距离：正在头顶，负在头底
                double v_dist = (target_point.z() - current_position.z());
